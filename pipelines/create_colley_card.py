...
delta_lp,details_lp = pyrankability.rank.solve(D,method='colley',cont=True)
...
# Now solve BILP
cont = False
delta,details = pyrankability.rank.solve(D,method='colley',fix_x=fix_x,cont=cont)
...
try:
    cont = False
    other_delta,other_detail = pyrankability.search.solve_any_diff(D,orig_obj,orig_sol_x,method='colley')
...
if len(instance.solutions) > 1: # Multiple optimal
    ...
    outlier_deltas,outlier_details = pyrankability.search.solve_fixed_cont_x(D,delta,centroid_x,method='colley',minimize=False)
