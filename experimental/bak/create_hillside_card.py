...
if len(sys.argv) < 3:
    print("Usage: python create_hillside_card.py <D dataset id> <dataset id>")
    exit(0)

#group = sys.argv[1]
source_dataset_id = int(sys.argv[1])
dataset_id = int(sys.argv[2])
result_path = f'{dataset_id}_hillside_card.json'#sys.argv[2]
...
# Now solve BILP
cont = False
delta,details = pyrankability.rank.solve(D,method='hillside',fix_x=fix_x,cont=cont)
...
try:
    cont = False
    other_delta,other_detail = pyrankability.search.solve_any_diff(D,orig_obj,orig_sol_x,method='hillside')
...
if len(instance.solutions) > 1: # Multiple optimal
    ...
    outlier_deltas,outlier_details = pyrankability.search.solve_fixed_cont_x(D,delta,centroid_x,method='hillside',minimize=False)
    instance.add_solution(outlier_details['perm'])
    instance.outlier_solution = outlier_details['perm']
    
    centroid_deltas,centroid_details = pyrankability.search.solve_fixed_cont_x(D,delta,centroid_x,method='hillside',minimize=True)
